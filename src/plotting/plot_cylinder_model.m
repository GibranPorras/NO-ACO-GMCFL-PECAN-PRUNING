...
if strcmp(Color,'order')
  ...
  Bpodar = cylinder.podar; %entra aqui y agregamos el campo podar
end
...
if strcmp(Color,'order')
  ...
  % primer color era igual a: 0.00 0.00 1.00 (rgb), segundo: 0.00 0.50
  % 0.00, color cafe: 0.38  0.25  0.14 
  col = [
    0.00  0.00  0.00
    0.96  0.36  0.28
    1.00  0.00  0.00
...
for i = 1:nc
  ...
  if Color == 1
    fvd(f:f+n-1,:) = repmat(col(Bpodar(i) + 1,:),[n 1]); %accede al 1 y al 2
  else
...
%%figure(fig)
plot3(fig,Vert(1,1),Vert(1,2),Vert(1,3))
patch(fig,'Vertices',Vert,'Faces',Facets,'FaceVertexCData',fvd,'FaceColor','flat', 'EdgeColor', 'none')
%%alpha(alp)
%%axis equal
%%grid on
%%view(-37.5,30)
...
%lighting gouraud %gouraud la original
end