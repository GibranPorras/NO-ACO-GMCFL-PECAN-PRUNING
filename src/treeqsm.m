...
function QSM = treeqsm(P,inputs, n_hormigas, hormigas)
...
QSM = struct('cylinder',{},'branch',{},'treedata',{},'rundata',{},...
    'pmdistance',{},'triangulation',{}, 'units', {}, 'segment2', {}, 'cover2', {});
  ...
  for i = 1:na
    ...
    for j = 1:ni
      ...
      if ~isempty(cylinder.radius)
        % matriz_dependencias = construir_matriz_dependencia(cylinder);
        disp("tiempo inicial");
        display(tic);
       for ant = 1:n_hormigas %% este for yo lo agregue 
        %% modificar cilindros si prueba > 0
           if n_hormigas > 1
            %primarias = find(cylinder.BranchOrder > 0);
            %podar = primarias(randi(length(primarias),30,1));
            %hormigas = GenPoblacionInicial(cylinder);
            hormiga = find(hormigas(ant, :) == 1);
            ramas = length(cylinder.BranchOrder);
            ramas_podar = [];
            indice = 1;
            if cylinder.extension(hormiga - 1) > 0
                %cylinder.extension(podar - 1) = 0;
            end

            for rp = 1: length(hormiga)
                r_podar = hormiga(rp);
                for r = r_podar: ramas
                    if cylinder.extension(r) == 0
                        ramas_podar(indice, :) = r;
                        indice = indice + 1;
                        break;
                    else
                        ramas_podar(indice, :) = r;
            ...
            aux_ramas_podar = [];
            %aux_ramas_podar = cat(1, aux_ramas_podar, ramas_podar);
            for d = 1: length(ramas_podar)
                idx_dep_poda = find(matriz_dependencias(ramas_podar(d),:) == 1);
                aux_ramas_podar = cat(2, aux_ramas_podar, idx_dep_poda);
            end
            
            aux_ramas_podar = cat(1, ramas_podar, aux_ramas_podar.');
            cylinder.podar(aux_ramas_podar, :) = 1;
           end
        ...
        [treedata,triangulation, units] = tree_data(cylinder,branch,trunk,inputs);
        Time(10) = toc-Time(9);
        if inputs.disp == 2
        ...
        if inputs.savemat
          ...
          %save_nogal_inicial(QSM(1), nombre_archivo);
        end
        % text-format (.txt)
        % if inputs.savetxt
        %   if nd > 1 || na > 1 || ni > 1
        ...
        %% Plot models and segmentations, poner a 1 para que grafique
        if inputs.plot >= 1
          if inputs.Tria
            plot_models_segmentations(P,cover2,segment2,cylinder, ant,trunk,...
                triangulation)
          else
            % plot_models_segmentations(P,cover2,segment2,cylinder, prueba)
          end
          if nd > 1 || na > 1 || ni > 1
            pause
          end
        ...
        if n_hormigas > 1
            cylinder.podar(aux_ramas_podar, :) = 0;
        end
      end%% termina for de pruebas
        disp("tiempo final");
        disp(toc);
      end
    end
