...
function [treedata,triangulation, Units] = tree_data(cylinder,branch,trunk,inputs)
...
No_podados = cylinder.podar ~= 1; % cilindros no podadas

%% Tree attributes from cylinders
% Volumes, areas, lengths, branches
treedata.TotalVolume = 1000*pi*Rad(No_podados).^2'*Len(No_podados);
treedata.TrunkVolume = 1000*pi*Rad(Trunk).^2'*Len(Trunk);
treedata.BranchVolume = 1000*pi*Rad(~Trunk & No_podados).^2'*Len(~Trunk & No_podados);
bottom = min(cylinder.start(No_podados,3));
[top,i] = max(cylinder.start(No_podados,3));
if cylinder.axis(i,3) > 0
  top = top+Len(i)*cylinder.axis(i,3);
end
treedata.TreeHeight = top-bottom;
treedata.TrunkLength = sum(Len(Trunk));
treedata.BranchLength = sum(Len(~Trunk & No_podados));
treedata.TotalLength = treedata.TrunkLength+treedata.BranchLength;
ramas_no_vacias = branch.volume ~= 0; % ramas que no excisten en el txt
NB = length(branch.order(ramas_no_vacias)); % number of branches
...
treedata.BranchArea = 2*pi*sum(Rad(~Trunk & No_podados).*Len(~Trunk & No_podados));
treedata.TotalArea = 2*pi*sum(Rad(No_podados).*Len(No_podados));
...
for i = 1:nc
  ...
  if cylinder.podar(i) == 1
      P(t,:) = NaN;
  elseif cylinder.branch(i) == 1
...
branches1 = (1:1:nb)';
branches1 = branches1(branch.order == 1 & branch.volume ~= 0); % 1st-order branches
nb = length(branches1);
nc = size(Sta,1);
ind = (1:1:nc)';
for i = 1:nb
  %disp(['tamaño de ind ' num2str(length(ind))]);
  %probando = cylinder.branch == branches1(i);
  %disp(['tamaño de probando' num2str(length(probando))]);
...
if b > 0
  ...
  Ind = (1:1:nb)';
  chi = Ind(branch.parent == b & branch.volume ~= 0);
  B = b;
  while ~isempty(chi)
    B = [B; chi];
    n = length(chi);
  ...
  BaseHeight = max(Sta(No_podados,3)); % Height of the crown base
  for i = 1:length(B)
    %disp(['tamaño de ind ' num2str(length(ind))]);
    %probando = (cylinder.branch == B(i)) & (cylinder.podar ~= 1);
    %disp(['tamaño de probando' num2str(length(probando))]);
    C = ind((cylinder.branch == B(i)) & (cylinder.podar ~= 1)); %modifique para que evalue podar
    ht = min(Tip(C,3));
    hb = min(Sta(C,3));
    h = min(hb,ht);
    if h < BaseHeight
